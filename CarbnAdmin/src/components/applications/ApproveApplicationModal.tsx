...
import { z } from "zod";
import { approveBetaApplication } from "../../api/betaApplicationsApi";
...
export default function ApproveApplicationModal({
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      const response = await approveBetaApplication({
        applicationId,
      ...
      if (response.email_sent === false) {
        setSuccessMsg(`${response.message} (Note: invitation email was not sent)`);
      }
      ...
      setError(getApiErrorMessage(err, "Could not approve application."));
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-[hsl(var(--card))] rounded-xl shadow-2xl w-full max-w-md border border-[hsl(var(--border))]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[hsl(var(--border))]">
          <h2 className="text-base font-semibold text-[hsl(var(--foreground))]">Approve Application</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-5 space-y-4">
          <p className="text-sm text-[hsl(var(--muted-foreground))]">
            Approve <span className="font-medium text-[hsl(var(--foreground))]">{applicantName}</span> for CARBN Founding Beta. An invitation email will be sent automatically.
          </p>
